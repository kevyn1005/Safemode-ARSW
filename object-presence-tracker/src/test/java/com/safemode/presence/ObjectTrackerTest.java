...
class ObjectTrackerTest {
    ...
    void registraElObjetoCuandoQuedaQuietoElTiempoSuficiente() {
        ...
        tracker.onFrame(List.of(suitcase(100, 100)));
        ...
        tracker.onFrame(List.of(suitcase(105, 95))); // dentro de la tolerancia de movimiento
    ...
    void noRegistraUnObjetoQueSigueMoviendose() {
        ...
        int x = 100;
        for (int i = 0; i < 6; i++) {
            tracker.onFrame(List.of(suitcase(x, 100)));
    ...
    void marcaRetiradoUnObjetoEnReposoQueDesapareceConUnaPersonaCerca() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            tracker.onFrame(List.of(person(120, 95)));
        }
    ...
    void marcaRetiradoSinPersonaCercaSiNadieEstabaAlLado() {
        ...
        for (int i = 0; i < 3; i++) {
            clock.advance(Duration.ofMillis(500));
            tracker.onFrame(List.of()); // escena vacia, nadie cerca
        }
    ...
    void unObjetoEnReposoQueSeMueveBruscamenteSeMarcaRetiradoDeInmediato() {
        ...
        clock.advance(Duration.ofMillis(500));
        tracker.onFrame(List.of(suitcase(400, 100))); // salto grande: alguien la levanto y la movio
